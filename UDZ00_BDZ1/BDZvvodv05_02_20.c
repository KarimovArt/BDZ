...
EEMEM unsigned int time[]={10,200,10000};
//выходы(мл.тетрада) подключенные ко входам
EEMEM unsigned char o[NUMINP]=   {0b00000100,0b00000100,0b00001001,0b00001010,0b00001001,0b00001010,0b00001001,0b00000000};	//входы [о0,о1,о2,о3,ц0,ц1,ц2,ц3]
//входы подтверждения.Только ЦВХ(младшая тетрада)
EEMEM unsigned char cin[NUMINP]= {0b00000000,0b00000000,0b00001000,0b00001000,0b00001000,0b00001000,0b00001000,0b00000000};
//режимы подтверждения (для ЦВХ)(0-mtz 1-urov)
EEMEM unsigned char cmod[NUMINP]={0b00000000,0b00000000,0b00000000,0b00000000,0b00000000,0b00000000,0b00001000,0b00000000};
...
{	//флаги:ACT,FLT,ALM,MOD;адреса (в ЕЕПРОМе) входов и типов подтверждений,адрес выходов,счетчики обработчиков
	{0,0,0,0,0,&cin[0],&cmod[0],&o[0],0},
	{0,0,0,0,0,&cin[1],&cmod[1],&o[1],0},
	{0,0,0,0,0,&cin[2],&cmod[2],&o[2],0},
	{0,0,0,0,0,&cin[3],&cmod[3],&o[3],0},
	{0,0,0,0,0,&cin[4],&cmod[4],&o[4],0},
	{0,0,0,0,0,&cin[5],&cmod[5],&o[5],0},
	{0,0,0,0,0,&cin[6],&cmod[6],&o[6],0},
	{0,0,0,0,0,&cin[7],&cmod[7],&o[7],0}
};
...
for(unsigned char i=0;i<NUMTIM;i++)
{
	...
	case T_ON:
	{
		...
		if (in[timer[i]->Nin].CONF_ALM) sendAlarm(1);
		else sendAlarm(0);
...
//uart_putchar(i);
	...
	if(outs !=0)
	{
		if(confirmInp==0 || MTZ_TIME == 0)  {createHandler(i,0,NOCONF);}	//срабатывания без подтверждений
...
{//uart_putchar('t');
	unsigned char i;
	static unsigned char flt[NUMINP];
...
static inline void updCRC (unsigned char *a_CRC,unsigned char *start,unsigned char lenght)
{
	...
	{
		asm("cli");
		*a_CRC= _crc_ibutton_update(*a_CRC,eeprom_read_byte(start+i));
		asm("sei");
	}
}
...
//clrBit(PORTD,3);
	flag.update=1;
	for(unsigned char k=0;k<NUMOPTINP;k++)
		if(in[k].ACT != chkBit(READOPTINPUT,k))
		{
			flag.trigger=1;in[k].ACT = chkBit(READOPTINPUT,k);
		}
